...
export default function Chronicles({ chronicles }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
          {chronicles.map(({ id, title, excerpt, date, tags }) => (
...
export async function getStaticProps() {
  ...
  // Filter for posts with a specific tag, e.g. "chronicle"
  const chronicles = allPostsData.filter(post =>
    post.tags && post.tags.includes('chronicle')
  );
  return {
    props: {
      chronicles,
